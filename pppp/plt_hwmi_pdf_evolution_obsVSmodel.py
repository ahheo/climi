...
def main():
    ...
    fnf = odir + cfg['v'] + '_evl_CMIP5vsCORDEXvsEOBS' + cfg['fn_pdf']
    idir = cfg['root'] + cfg['experiment'] + '/' + cfg['res']

    ##############################hist

    fig = plt.figure(figsize = (5, 8))
    fig.subplots_adjust(hspace=0.15, wspace=0.075,
    ...
    for i, p0 in enumerate(p0s):
        ...
        #obs
        ddir = idir + 'obs/'
        [il0, el0] = pdf_iANDe_(ax0, color, cfg, ddir, cfg['dn_obs'], p, True)
        el.append(el0)
        lg.append(rPeriod_(p, True))
        ##cmp
        ddir = idir + 'cmip5/hist/'
        pdf_iANDe_(ax1, color, cfg, ddir, cfg['dn_cmp'], p, True)
        ##cdx
